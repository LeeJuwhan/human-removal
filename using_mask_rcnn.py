...
import shutil
from matcher import stackImagesKeypointMatching as km
from test import inpainting
import make_mask
...
def video2image(input_path):
    ...
    while (vidcap.isOpened()):
        ...
        if (int(vidcap.get(1)) % int(v_fps / 2) == 0):            
        # if (int(vidcap.get(1))):        
            count = count + 1
            video_frame[count] = image 
            cv2.imwrite("./images/%d.png"%count, image)
            count += 1            
            #for bandicam cut 4 sec
            # if(int(vidcap.get(1)) >= v_length -int(v_fps * 4)):
            if(int(vidcap.get(1)) >= v_length -int(v_fps /2)):
                vidcap.release()
...
def main():
    ...
    # matcher_image = km("./output/")
    # cv2.imwrite("./matcher_results.png",matcher_image)
    make_mask.main("./matcher_output/","./roi_path/mask/","./roi_path/mask_roi/","./roi_path/image_roi/","./results/")
